package com.neilren.neilren4j.modules.frielink.service;
...
import com.neilren.neilren4j.common.utils.*;
...
public class FrielinkService extends BaseService {
     ...
     * @return
     */
    public List<Frielink> getValidFrieLinks() {
        List<Frielink> frielinks = null;
        frielinks = (List<Frielink>) memcachedManager.get(memcachedFrieLinkKey);
        if (frielinks == null || frielinks.size() < 1) {
            frielinks = frielinkDao.selectValidFrieLinks();
            memcachedManager.set(memcachedFrieLinkKey, frielinks, Global.MemcachedExpire);
        }
        return frielinks;
    }

    public Frielink getFrieLinkByDomain(String domain) {
        return frielinkDao.selectFrieLinkByDomain(domain);
    }
     ...
     * @return
     */
    public Frielink saveFrieLinkByForm(Forms forms) {
        Frielink frielink = new Frielink();
        ...
        frielink.setAdd_date(new Date());
        frielink.setState(0);
        frielink.setBlack_white(0);
        frielink.setRemark("自助申请");
        frielinkDao.insertFrieLink(frielink);
        return frielink;
    }
     ...
     * @param frielink
     */
    public void firstAudit(Frielink frielink) {
     ...
     * @param frielink
     */
    public void firstAuditThread(Frielink frielink) {
    ...
    void regularVisit() {
        ...
        //获取正在展示的友情链接
        List<Frielink> showingFrielink = frielinkDao.selectShowingFrieLinks();
        for (Frielink frielink : showingFrielink) {
            //检查是否有回链
            try {
                ...
                if (html != null) {
                    if (html.indexOf("https://www.neilren.com") == -1 && html.indexOf("https://neilren.com") == -1 &&
                        ...
                        frielink.setState(0);
                        frielink.setLast_check_time(new Date());
                        frielinkDao.updateByFrieLink(frielink);
                        //发送通知邮件
        ...
        //获取隐藏的友情链接
        List<Frielink> hideFrielink = frielinkDao.selectHideFrieLinks();
        for (Frielink frielink : hideFrielink) {
            boolean pass = false;
            //检查注册时间是否大于六个月
            try {
    ...
    private void sendEmail(Frielink frielink, String title, String Sabstract, String content) {
    ...
    private class FirstFrielinkAuditThread extends Thread {
         ...
         */
        FirstFrielinkAuditThread(Frielink frielink) {
        ...
        public void run() {
            ...
            //是否在黑名单中
            List<Frielink> blackFrielinks = frielinkDao.selectBlackFrieLinks();
            for (Frielink blackFrielink : blackFrielinks) {
